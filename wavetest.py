...
def wavelet_plot(var, time, data, dtmin, result, **kwargs):
    ...
    TODO:
        ...
        DESTACAR A AREA QUE EU TENHO INTERESSE (3 E 30 DIAS) OK
    '''
    ...
    extra_plot = kwargs.get('extra_plot', None)
    if extra_plot[0]:
        ...
        ax2 = plt.subplot(gs1[3:8, :])  # , axisbg='#C0C0C0')
        ax6 = plt.subplot(gs1[2, :])  # , axisbg='#C0C0C0')
    ...
    ax1.set_ylabel(kwargs.get('ylabel_data', 'Amplitude'), fontsize=12)
    ...
    if extra_plot[0]:
        ax6.plot(time, extra_plot, color='k', label=kwargs.get('extra_lbl', 'None'))
        ax6.legend()
        ax6.set_ylim(-3,None)
        ax6.set_yticks([0, int(extra_plot.max())/2])
        ax6.xaxis.set_visible(False)

        # ax1.set_ylabel(kwargs.get('extra_lbl', 'None'), fontsize=10)
        if xmin is not None or xmax is not None:
